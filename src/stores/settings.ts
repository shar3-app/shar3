...
export const onSettingChanges = async (
  ...
  return await (await settingsStore()).onKeyChange(key, callback);
};

export const getSetting = async (key: keyof Settings): Promise<any> => {
  return await (await settingsStore()).get(key);
};

export const getSettings = async (): Promise<Settings> => {
  const entries = await (await settingsStore()).entries();
  return {
    ...defaultSettings,
