...
const ContextMenu = () => {
	...
	useEffect(() => {
		...
		setOverflowX(
			position.x + MENU_WIDTH > (contextMenuContainer?.offsetWidth ?? 0),
		);
		setOverflowY(
			position.y + MENU_HEIGHT > (contextMenuContainer?.offsetHeight ?? 0),
		);
	...
	return (
		...
		>
			<div
				...
				id="contextMenu"
				style={{
					[overflowX ? "right" : "left"]: overflowX
						? window.innerWidth - position.x
						: position.x,
					[overflowY ? "bottom" : "top"]: overflowY
